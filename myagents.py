...
def build_graph(state_space):
    ...
    for label, location in maze_map_locations.items():
        graph.add_node(label)
        node_labels[label] = label
        node_colors[label] = 'white'

    initial_node_colors = dict(node_colors)

    node_label_pos = {k:[v[0],v[1]] for k,v in maze_map_locations.items()}
    ...
    while cnode.state != start_location:    
        cnode = cnode.parent  
        solution_path.append(cnode)
    ...
    nx.draw(graph, pos = maze_map_locations, node_color = [node_colors[node] for node in graph.nodes()])

    # draw labels for nodes
    node_label_handles = nx.draw_networkx_labels(graph, pos = node_label_pos, labels = node_labels, font_size = 7)
    # add a white bounding box behind the node labels
    [label.set_bbox(dict(facecolor='white', edgecolor='none')) for label in node_label_handles.values()]

    # add edge lables to the graph
    nx.draw_networkx_edge_labels(graph, pos = maze_map_locations, edge_labels=edge_labels, font_size = 14)
